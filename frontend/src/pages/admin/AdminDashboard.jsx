...
const [bulkForm, setBulkForm] = useState({
  ...
  }, [events, categories, nominations, successfulTx, organizations])
...
function List({ items, render }) {
  return <div className="space-y-3 max-h-[660px] overflow-y-auto pr-1">{items.length === 0 ? <p className="text-slate-500 font-bold">No records yet.</p> : items.map(item => <div key={item.id} className="w-full min-w-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">{render(item)}</div>)}</div>
}
