...
async function getCommitteeData() {
  const [categoryRes, nominationRes, displaySettingsRes] = await Promise.all([
    supabase
      .from('categories')
      .select('*')
      .eq('is_active', true)
  ...
  if (displaySettingsRes.error) throw displaySettingsRes.error

  return {
    categories: categoryRes.data || [],
    nominations: nominationRes.data || [],
    displaySettings: displaySettingsRes.data || null,
  }
}
...
export default function CommitteeDashboard() {
  ...
  const leaderboardVisible = data?.displaySettings?.leaderboard_visible !== false
  ...
  const leaders = leaderboardVisible ? categoryGroups.filter(item => item.leader) : []

  useEffect(() => {
    if (!leaderboardVisible && activeTab === 'leaders') setActiveTab('overview')
  }, [leaderboardVisible, activeTab])
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-950">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-4 space-y-4">
          ...
          <nav className="overflow-x-auto pb-1">
            <div className="flex gap-2 min-w-max">
              {dashboardTabs.filter(([key]) => leaderboardVisible || key !== 'leaders').map(([key, Icon, label]) => (
                <button
                  key={key}
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        ...
        {activeTab === 'leaders' && (
          <LeadersTab isLoading={isLoading} categoryGroups={leaderboardVisible ? categoryGroups : []} />
        )}
...
function OverviewTab({ summary, summaryLoading, categories, nominations, leaders, leaderboardVisible }) {
  return (
    <section className="space-y-6">
      ...
      {leaderboardVisible && <section className="rounded-[2rem] bg-white border border-slate-200 p-5 sm:p-6 shadow-lg">
